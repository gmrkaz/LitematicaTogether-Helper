...
async function ensureInfrastructure(g,{forcePanel=false}={}){
 ...
 const staffAllow=[PermissionFlagsBits.ViewChannel,PermissionFlagsBits.SendMessages,PermissionFlagsBits.ReadMessageHistory,PermissionFlagsBits.AttachFiles,PermissionFlagsBits.EmbedLinks,PermissionFlagsBits.SendMessagesInThreads,PermissionFlagsBits.ManageThreads];
 ...
 if(!cfg.ticketHubReady){
   ...
   if(oldInternal){
     ...
     await oldInternal.permissionOverwrites.set(staffPO).catch(()=>{});
   }
 }

 let ticketHub=channelByName(g,'support-staff',[ChannelType.GuildText]);
 if(!ticketHub)ticketHub=await g.channels.create({name:'support-staff',type:ChannelType.GuildText,parent:staffCat.id,permissionOverwrites:staffPO,topic:'Private Support ticket hub. Ticket owners only see this hub while their ticket is open.'});
 else {await ticketHub.setParent(staffCat.id,{lockPermissions:false}).catch(()=>{});await ticketHub.permissionOverwrites.set(staffPO).catch(()=>{});}

 let archive=channelByName(g,'ticket-archive',[ChannelType.GuildText])||await g.channels.create({name:'ticket-archive',type:ChannelType.GuildText,parent:staffCat.id,permissionOverwrites:staffPO,topic:'Closed Support transcripts. Staff only.'});
 ...
 Object.assign(cfg,{supportChannelId:support.id,modLogChannelId:mod.id,supportStaffChannelId:ticketHub.id,ticketArchiveChannelId:archive.id,supportRoleId:supportRole.id,ticketHubReady:true});db.save();
