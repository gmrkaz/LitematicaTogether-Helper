...
const STAFF = ['Owner','Co-Owner','Administrator','Moderator','Developer','Support Team'];
...
const isStaff = m => m && (m.guild.ownerId === m.id || m.permissions.has(PermissionFlagsBits.Administrator) || m.permissions.has(PermissionFlagsBits.ManageThreads) || m.roles.cache.some(r => STAFF.includes(r.name)));
...
const commands = [
  ...
  new SlashCommandBuilder().setName('ticket-add').setDescription('Add a member to this ticket').addUserOption(o=>o.setName('user').setDescription('User').setRequired(true)),
  ...
  new SlashCommandBuilder().setName('timeout').setDescription('Timeout a member').setDefaultMemberPermissions(PermissionFlagsBits.ModerateMembers).addUserOption(o=>o.setName('user').setDescription('Member').setRequired(true)).addStringOption(o=>o.setName('duration').setDescription('10m, 2h, 1d').setRequired(true)).addStringOption(o=>o.setName('reason').setDescription('Reason').setRequired(true).setMaxLength(500)),
...
module.exports = { STAFF, trunc, clean, base, isStaff, duration, commands, donationPayload };
