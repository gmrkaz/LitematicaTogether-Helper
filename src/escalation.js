  ...
  return [
    new ActionRowBuilder().addComponents(
      ...Object.entries(ROLE_CALLS).map(([customId, cfg]) =>
        new ButtonBuilder().setCustomId(customId).setLabel(cfg.label).setStyle(ButtonStyle.Secondary)
      ),
    ),
    new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(OWNER_CALL_ID).setLabel('Ametist1k').setStyle(ButtonStyle.Primary),
    ),
  ];
}
...
async function callOwnerBridge(i, ticket) {
  const url = process.env.OWNER_BOT_WEBHOOK_URL;
  const secret = process.env.OWNER_BOT_SHARED_SECRET;
  if (!url || !secret) {
    return i.reply({
      content: 'Owner Telegram bot bridge is not configured yet. Set OWNER_BOT_WEBHOOK_URL and OWNER_BOT_SHARED_SECRET on the Discord Helper host.',
      ephemeral: true,
    });
  }

  const payload = {
    type: 'ticket_owner_call',
    guildId: i.guild.id,
    guildName: i.guild.name,
    channelId: i.channelId,
    channelName: i.channel.name,
    requestedById: i.user.id,
    requestedByTag: i.user.tag,
    ticketOwnerId: ticket.userId,
    ticketUrl: `https://discord.com/channels/${i.guild.id}/${i.channelId}`,
    createdAt: Date.now(),
  };
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'x-owner-bot-secret': secret,
    },
    body: JSON.stringify(payload),
  });
  const body = await response.json().catch(() => null);
  if (!response.ok || !body?.ok) {
    throw new Error(`Owner Telegram bridge failed: ${body?.error || response.status}`);
  }
...
async function handleTicketEscalation(i) {
  ...
  if (isOwnerCall) await callOwnerBridge(i, ticket);
...
module.exports = { escalationRows, handleTicketEscalation, installEscalationHook };
